...
def unet_train():
    ...
    model.fit(X_train, y_train, epochs=100, batch_size=15)  # The epochs and batch_size should be adjusted according to personal circumstances. The batch_size should not be too large, otherwise the memory will overflow easily.
     #My 11G video memory can only be set to about 15-20, and the final loss of my training is reduced to about 250, and the acc is about 95%.
    model.save('unet.h5')
    print('unet.h5 saved successfully!!!')


def unet_predict(unet, img_src_path):
    img_src = cv2.imdecode(np.fromfile(img_src_path, dtype=np.uint8), -1)  # When reading from a Chinese path, use
    # img_src=cv2.imread(img_src_path)
    if img_src.shape != (512, 512, 3):
        img_src = cv2.resize(img_src, dsize=(512, 512), interpolation=cv2.INTER_AREA)[:, :, :3]  # dsize=(width,height),[:,:,:3] is to prevent the image from being a 4-channel image, which cannot be reshaped later
    img_src = img_src.reshape(1, 512, 512, 3)  # The predicted picture shape is (1,512,512,3)

    img_mask = unet.predict(img_src)  # Prediction after normalization divided by 255
    img_src = img_src.reshape(512, 512, 3)  # Reshape the original image to 3D
    img_mask = img_mask.reshape(512, 512, 3)  # Reshape the predicted image to 3D
    img_mask = img_mask / np.max(img_mask) * 255  # Multiply by 255 after normalization
    img_mask[:, :, 2] = img_mask[:, :, 1] = img_mask[:, :, 0]  # The three channels remain the same
